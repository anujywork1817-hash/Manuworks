...
func main() {
	...
	// ─── 7. Init Gemini ──────────────────────────────────────────────────────────
	// Optional: if GEMINI_API_KEY isn't set (e.g. running locally with just a
	// Groq key), skip it instead of refusing to start. Gemini-backed features
	// (semantic search / embeddings) will report themselves as unavailable
	// rather than crashing the whole server.
	var geminiClient *gemini.Client
	if cfg.Gemini.APIKey == "" {
		logger.Warn("GEMINI_API_KEY not set - semantic search / embeddings disabled")
	} else {
		geminiClient, err = gemini.NewClient(ctx, &cfg.Gemini)
		if err != nil {
			logger.Warn("Failed to init Gemini client - semantic search / embeddings disabled", logger.Err(err))
			geminiClient = nil
		} else {
	...
	protected.Use(middleware.AuthRequired(cfg))
	{
		...
		protected.POST("/ai/draft", aiH.DraftGenericDocument)
