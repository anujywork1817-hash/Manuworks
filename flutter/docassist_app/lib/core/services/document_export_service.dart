...
class DocumentExportService {
  ...
  /// package falls back to its built-in Latin-only font and any non-Latin
  /// character (e.g. a Hindi translation) renders as empty tofu boxes.
  /// Cached after first load so repeated exports don't re-fetch fonts.
  static Future<pw.ThemeData>? _themeFuture;

  static Future<pw.ThemeData> _unicodeTheme() {
    return _themeFuture ??= () async {
      final results = await Future.wait([
        PdfGoogleFonts.notoSansRegular(),
        PdfGoogleFonts.notoSansBold(),
        PdfGoogleFonts.notoSansDevanagariRegular(), // Hindi, Marathi
      ...
      final fallback = results.sublist(2);
  ...
  /// share sheet (mobile/desktop), pre-filled so the user can pick their
  /// mail app. Used by the "Email" toolbar action.
  ///
  /// On web there is no share sheet and — crucially — no way for a webpage
  /// to attach a file to an email at all (mailto: links cannot carry
  /// attachments; this is a browser limitation, not something fixable
  /// here). The best available fallback: download the PDF (so the user has
  /// it locally) and open their default mail client with the subject/body
  /// pre-filled, so they only need to attach the file that was just saved.
  static Future<void> emailReport({
    ...
    final file = await saveReportPdf(title: title, content: content);
    if (file == null) {
      // web: saveReportPdf already downloaded '<title>.pdf' — open the
      // mail client so the user just has to attach it.
      final fileName = '${_safeFileName(title)}.pdf';
      final mailUri = Uri(
        scheme: 'mailto',
        query: 'subject=${Uri.encodeComponent(title)}&'
            'body=${Uri.encodeComponent('Please find the attached report: $title\n\n'
